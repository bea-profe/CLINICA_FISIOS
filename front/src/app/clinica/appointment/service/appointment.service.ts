...
export class AppointmentService {
  showAppointmentAttention(appointment_id: any) {
...
registerAppointment(data: any): Observable<any> {
  ...
  registerAttention(data: any) {
    ...
    // Constructing the URL for the appointment endpoint
    let URL=URL_SERVICIOS+"/appointment-attendance";
  
    // Making an HTTP POST request with the data and headers
    return this.http.post(URL,data,{headers: headers});
  }

  showAppointmentAtendance(appointment_id:string){

    let headers =new HttpHeaders({'Authorization':'Bearer '+this.authService.token});
    let URL=URL_SERVICIOS+"/appointment-attendance/"+appointment_id;
    return this.http.get(URL,{headers: headers});
